...
export function Passport({ journey, onClose, onResume, onRestart }: PassportProps) {
  ...
  return (
    <div className="overlay-in fixed inset-0 z-50 flex flex-col items-center justify-center bg-black/80 backdrop-blur-sm p-4">
      ...
      <div
        className={`relative transition-all duration-700 ease-in-out ${opened ? 'w-[90%] sm:w-[80%] md:w-[70%] max-w-3xl aspect-[1440/1024]' : 'w-[58%] max-w-[220px] aspect-[255/354] cursor-pointer sm:w-[40%] sm:max-w-xs'} ${(page === 2 && flipping !== 'backward') || flipping === 'forward' ? 'translate-x-[25%]' : 'translate-x-0'}`}
